...
export default function TestimonialsSection({ className }: { className?: string }) {
  ...
  return (
    <div className={`py-[40px] sm:py-[62px] relative ${className}`}>
      {isMobile ? (
        <div
          className={`relative flex flex-col items-center gap-[20px] w-full max-w-full overflow-hidden`}
        >
          {/* Carrusel */}
          <div
            ref={carouselRef}
        ...
        <div className="flex flex-wrap justify-center gap-[20px]">
          {testimonials.slice(0, 3).map((testimonial, index) => {
            ...
            return (
              <div
                key={index}
                className="flex h-[531px] w-[305px] items-center justify-center bg-white"
              >
                {isVideo ? (
                  <video
                    src={testimonial.src}
