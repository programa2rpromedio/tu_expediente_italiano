...
const Alert: React.FC<AlertProps> = ({
  ...
  const baseStyles = {
    container:
      "fixed top-4 h-[20px] inset-x-0 mx-auto z-50 max-w-[550px] w-full md:w-[550px] sm:w-[300px] xs:w-[200px] p-4 rounded-lg shadow-lg border flex items-center", // Añadido flex items-center
    content: "flex items-center justify-between w-full",
    text: "text-sm text-[#4C4C4C] text-[28px] text-weight-400",
    button: "ml-4 p-1 rounded-md focus:outline-none focus:ring-2",
  };

  const typeStyles: Record<AlertVariant, {
    container: string;
    text: string;
    button: string;
  }> = {
  ...
  const icons: Record<AlertVariant, JSX.Element> = {
    success: (
      <Check className="pr-2" width={"24"} height={"24"} color={"black"}/> 
    ),
    error: (
      <svg
    ...
    <div className={`${baseStyles.container} ${typeStyles[type].container}`}>
      <div className={baseStyles.content}>
        <div className="flex items-center"> 
          {icons[type]}
          <p className={`${baseStyles.text} ${typeStyles[type].text}`}>
            {message}
          </p>
        </div>
        {onClose && (
